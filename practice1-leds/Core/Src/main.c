...
int main(void)
{
  ...
  while (1)
  {
    if (fsm_state)
    {
      led_blink(LEDS_GPIO_Port, cbuffer_prev(&leds_cbuffer), 200);
    }
    else
    {
      led_blink(LEDS_GPIO_Port, cbuffer_next(&leds_cbuffer), 200);
    }
    
    /* Toggle LEDs sequence with USER BUTTON (without anti-bounce) */
